#!/usr/bin/env python3
# SplitGrid - benchmark asymmetric serialization (host stdlib vs child NumPy).
# Run from the repo root: python scripts/bench_serialization.py
"""Compare json_list, json+sg, pickle5, and pickle5+sg for host→child and child→host paths.

See splitgrid.codec for why the split_grid envelope exists (NumPy frombuffer).
"""
...
# deal @pre caps shape dim at 256 for pytest/CrossHair. Benches use 20_000×5.
try:
...
from splitgrid.codec import (  # noqa: E402
    BINARY_MIN_CELLS,
    MAX_BENCH_CELLS,
    ForceBinary,
    cell_count,
...
from tests.payload_codec_test_support import (  # noqa: E402
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="Use split_grid envelope when cell count is at least this (default from splitgrid.codec)",
    )
    ...
    print(
        "\nColumn guide:\n"
        "  direction     ingress = Host -> Child (stdlib pack, then child NumPy unpack).\n"
        "                egress = Child -> Host (NumPy pack, then host stdlib unpack).\n"
