...
"""Wire codec for host (plain Python) and child (NumPy) split-grid serialization.

Extracted from WriterAgent's ``plugin.scripting.payload_codec``. Host pack stays
NumPy-free; child unpack/pack may use NumPy. Cython flatten is optional.
...
from splitgrid.deal_shim import DEAL_MAX_SHAPE_DIM, DEAL_MAX_SHAPE_RANK, deal
...
def _verify_accelerator(fn2d: Any, fn1d: Any) -> bool:
    ...
    try:
        ...
        if not (
            ...
            log.warning("splitgrid: Cython 2D canary failed")
            return False
        ...
        if not (
            ...
            log.warning("splitgrid: Cython 1D canary failed")
            return False

        return True
    except Exception as e:
        log.warning("splitgrid: Cython canary exception: %s", e)
        return False
...
def load_cython_accelerator() -> None:
    ...
    # In-package compiled extension (splitgrid.pack). Missing .so/.pyd is a normal
    # fallback to the proven pure-Python flatten — do not require Cython at import.
    try:
        from splitgrid.pack import fast_flatten_grid_2d as _fn2d, fast_flatten_grid_1d as _fn1d
    ...
    if fn2d is not None and fn1d is not None:
        if _verify_accelerator(fn2d, fn1d):
            ...
            _CYTHON_ACCELERATOR_DISABLED = False
            log.debug("splitgrid: Cython accelerator (%s) verified and loaded", loc)
        else:
            _CYTHON_ACCELERATOR_DISABLED = True
            _CYTHON_ACCELERATOR_LOCATION = None
            log.warning("splitgrid: Cython accelerator found at %s but failed canary check; using pure Python", loc)
    else:
        _CYTHON_ACCELERATOR_DISABLED = True
        _CYTHON_ACCELERATOR_LOCATION = None
        log.debug("splitgrid: Cython accelerator not found, using pure Python")


def invalidate_host_cython_accelerator() -> None:
    """Drop in-process accelerator state after host natives were replaced on disk.

    Atomic replace of the compiled extension (new inode) can leave ``sys.modules``
    holding a stale ``splitgrid.pack`` mapping. Clear globals and module cache so
    the next load binds the new file.
    """
    global fast_flatten_grid_2d, fast_flatten_grid_1d, _CYTHON_ACCELERATOR_DISABLED, _CYTHON_ACCELERATOR_LOCATION
    ...
    for key in list(sys.modules):
        if key in ("splitgrid.pack",) or key.startswith("splitgrid.pack."):
...
def host_cython_status_line(*, reload: bool = False) -> str:
    """Human-readable Cython status line.

    Default is report-only (no import/reload). Pass ``reload=True`` after the
    compiled extension was replaced on disk.
    """
    if reload:
...
WriterAgent materializes this as CalcRange; SplitGrid unpacks the inner grid."""
...
def is_calc_range_payload(obj: Any) -> bool:
    # Codec wire guard (WriterAgent's calc_range.py has a parallel detector).
...
def _validate_rectangular_grid(grid_2d: list[list[Any]], ncols: int) -> None:
    ...
    for row in grid_2d:
        if len(row) != ncols:
            ...
            log.error("splitgrid: uneven row lengths in 2D grid: %s", row_lens)
    ...
    if is_2d:
        ...
        if fast_flatten_grid_2d is not None:
            try:
                ...
                log.debug("splitgrid: Cython accelerator failed, falling back to stdlib: %s", e)
        ...
        if fast_flatten_grid_1d is not None:
            try:
                ...
                log.debug("splitgrid: Cython 1D accelerator failed, falling back to stdlib: %s", e)
...
def host_pack_split_grid(
    ...
    log.debug(
        "splitgrid host_pack split_grid column_kinds=%s shape=%s cells=%s strings=%s raw_bytes=%s",
        column_kinds,
        shape,
        len(buf),
        len(strings),
...
def host_pack_data(
    ...
    *,
    ...
    try:
        ...
        log.debug("splitgrid host_pack json_list %s", describe_wire_value(out))
        return out
    except Exception:
        log.exception("splitgrid host_pack failed for grid %s", describe_wire_value(grid))
        raise
...
def host_pack_multi_data(
    ...
    *,
    ...
    log.debug(
        "splitgrid host_pack multi_data items=%s cells=%s",
        len(items),
...
def child_unpack_split_grid(envelope: dict[str, Any]) -> Any:
    ...
    try:
        ...
        if not strings:
            ...
            log.debug("splitgrid child_unpack split_grid optimized -> ndarray shape=%s dtype=%s", arr.shape, arr.dtype)
        ...
        log.exception("splitgrid child_unpack split_grid failed for envelope %s", describe_wire_value(envelope))
        raise
...
def _child_unpack_single_data(wire: Any) -> Any:
    ...
    if isinstance(unpacked, np.ndarray):
        ...
        if is_numeric_grid(grid):
            ...
            return arr
        log.debug("splitgrid child_unpack json_list as-is %s", describe_wire_value(unpacked))
        return grid
    return unpacked
...
def child_unpack_data(wire: Any) -> Any:
    ...
    For ``calc_range`` envelopes, unpacks the inner grid (no CalcRange wrapper).
    Bare grids become ndarray/list; ``multi_data`` unpacks each item.
    """
    # crosshair: off
    try:
        # WriterAgent wraps ranges as CalcRange; SplitGrid unpacks the inner grid only.
        if is_calc_range_payload(wire):
            return child_unpack_data(wire.get("data"))
        if is_multi_data(wire):
            items = wire.get("items") or []
        ...
        log.exception(
            "splitgrid child_unpack failed for wire %s",
...
def child_pack_split_grid(arr: Any) -> dict[str, Any]:
    ...
    try:
        ...
        log.debug(
            "splitgrid child_pack split_grid column_kinds=%s shape=%s cells=%s raw_bytes=%s",
            column_kinds,
            wire_arr.shape,
            wire_arr.size,
            len(envelope["buffer"]),
        ...
        log.exception(
            "splitgrid child_pack split_grid failed for value %s",
    ...
    *,
    ...
    try:
        if isinstance(result, np.ndarray):
            ...
            log.debug(
                "splitgrid child_pack json_list egress ndarray shape=%s (below_threshold)",
                shape,
            )
        ...
        if isinstance(result, (list, tuple)):
            ...
            if should_use_binary_envelope(grid_shape, min_cells=min_cells, force=force):
                return host_pack_split_grid(grid)
            out = grid_from_nested_list(grid)
            log.debug("splitgrid child_pack json_list egress %s", describe_wire_value(out))
            return out
        return result
    except Exception:
        log.exception(
            "splitgrid child_pack_result failed for value %s",
