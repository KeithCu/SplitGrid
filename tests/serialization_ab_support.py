...
"""Shared fixtures and oracles for codec-level A/B serialization tests (no venv worker)."""
...
def assert_cython_vs_python_flatten_parity(
    grid: list[Any] | list[list[Any]],
    *,
    label: str = "",
) -> None:
    """Compare Cython vs pure-Python flatten components when the extension is loaded."""
    if pc.fast_flatten_grid_2d is None and pc.fast_flatten_grid_1d is None:
        return

    with cython_accelerator_context(enabled=True):
        cython_wire = host_pack_data(grid, force="always")
    with cython_accelerator_context(enabled=False):
        python_wire = host_pack_data(grid, force="always")

    assert cython_wire["__wa_payload__"] == python_wire["__wa_payload__"], label
    assert cython_wire["shape"] == python_wire["shape"], label
    assert cython_wire["column_kinds"] == python_wire["column_kinds"], label
    assert cython_wire["strings"] == python_wire["strings"], label
    assert cython_wire["buffer"] == python_wire["buffer"], f"{label}: buffer bytes differ"
from tests.serialization_cases import SerializationCase, all_serialization_cases
...
def prepare_grid(case: AbGridCase) -> list[Any] | list[list[Any]]:
    """Return the fixture grid as-is (already rectangular; no CalcRange wrapping)."""
    return case.grid
