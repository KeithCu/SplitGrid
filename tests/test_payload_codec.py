...
import splitgrid.codec as payload_codec
...
def test_column_kinds_from_cell_types():
    from splitgrid.codec import column_kinds_for_grid
...
def test_uniform_unpack_uses_full_column_kinds_on_wire():
    ...
    from splitgrid.codec import envelope_uniform_column_kind
...
def test_child_pack_integer_ndarray_sets_column_kinds():
    np = pytest.importorskip("numpy")
    from splitgrid.codec import child_pack_result
...
def test_iter_split_grid_cells_row_major_order() -> None:
    ...
    from splitgrid.codec import _iter_split_grid_cells
...
def test_get_cython_status_info() -> None:
    ...
    from splitgrid.codec import get_cython_status_info, host_cython_status_line
...
def test_cython_canary_failure_disables_accelerator() -> None:
    ...
    from splitgrid.codec import _verify_accelerator
...
def test_invalidate_host_cython_accelerator_clears_globals_and_modules() -> None:
    ...
    saved_pack = sys.modules.get("splitgrid.pack")
    fake = types.ModuleType("splitgrid.pack")
    fake.fast_flatten_grid_2d = object()
    sys.modules["splitgrid.pack"] = fake
    ...
    try:
        ...
        assert "splitgrid.pack" not in sys.modules
    finally:
        ...
        if saved_pack is not None:
            sys.modules["splitgrid.pack"] = saved_pack
        else:
            sys.modules.pop("splitgrid.pack", None)
